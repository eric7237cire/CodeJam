...
int main() 
{
	...
	while( cin >> cmdType )
	{
		...
		if (cmdType == '!')
		{
			int q1, q2;
			string s1, s2;
			cin >> q1 >> s1 >> cmdType >> q2 >> s2;
			int id1, id2;
			id1 = getId(mapNameId, mapNames, s1, mapNames.size());
			id2 = getId(mapNameId, mapNames, s2, mapNames.size());
			
			edges[id1][id2] = WeightedEdge<ii>( id1, id2, ii(q1, q2) );
			edges[id2][id1] = WeightedEdge<ii>( id2, id1, ii(q2, q1) );
		...
		if (cmdType == '?')
		{
			...
			int set1 = uf.findSet(id1);
			int set2 = uf.findSet(id2);
			...
			//Location, quantity
			queue<pair<int, ii> > toVisit;
			memset( visited, 0, sizeof visited );
			
			toVisit.push( mp(id1, ii(1, 1)) );
			
			while( ! toVisit.empty() )
			{
				pair<int, ii> top = toVisit.front();				
				toVisit.pop();
				...
				int curVert = top.first;
				ii curRatio = top.second;
				
				//printf("Cur node %s.  Conv Ratio %d / %d\n", mapNames[curVert].c_str(), 
					// curRatio.first, curRatio.second);
				
				if (curVert == id2)
				{
					printf("%d %s = %d %s\n", curRatio.second,s1.c_str(), 
					 curRatio.first, s2.c_str());
				...
				FOR(nextVert, 0, 60)
				{
					ii edgeRatio = edges[curVert][nextVert].weight;
					
					//edge does not exist 
					if (edges[curVert][nextVert].weight.first == 0)
						continue;
						
					/* we want find convertion ratio for v, we have u 
					so multiply by 
					...
					*/
					ii newRatio = mp( curRatio.first * edgeRatio.second,
						curRatio.second * edgeRatio.first );
						
					int comFac = gcd(newRatio.first, newRatio.second);
					newRatio.first /= comFac;
					newRatio.second /= comFac;
					
					toVisit.push( mp( nextVert, newRatio ) );
				}
			}
		}
