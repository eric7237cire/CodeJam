...
int main2()
{
	...
	while(true)
	{
		...
		//vector<BigInt> dp; // dp[i] means max product of seq ending at vertex [i]
		
		
		BigInt curPos( a[0] > 0 ? a[0] : 0 );
		BigInt curNeg( a[0] < 0 ? a[0] : 0 );
		...
		//cout << "curPos " << curPos << " curNeg " << curNeg << " best " << best << endl;
		...
		for(int i = 1; i < a.size(); ++i)
		{
			...
			//cout << "a[" << i << "]= " << a[i] << " curPos " << curPos << " curNeg " << curNeg << " best " << best << endl;
		}	
