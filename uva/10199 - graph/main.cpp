...
int main() {
	...
	while( scanf("%d", &V) == 1 && V )
	{
		...
		FOR(v, 0, V)
		{
			...
			cin >> name;
			//cout << name;
			getId( mapNameId, mapNames, name, mapNames.size() );
		}
		...
		//	cout << name1 << endl;
			//cout << name2 << endl;
			int id1 = getId( mapNameId, mapNames, name1, mapNames.size() );
			int id2 = getId( mapNameId, mapNames, name2, mapNames.size() );
			assert(id1 < V);
			assert(id2 < V);
			assert(adjList.size() == V);
			adjList[id1].pb(id2);
			adjList[id2].pb(id1);
		}
