...
DEC2::DEC2(const DEC2& y, int l) {
	*this = DEC2::DEC2(y);
	length = l;
}
DEC2::operator =(const DEC2& y) {
	if (this == &y)
		return *this;
	digits.clear();
	postdec.clear();
...
DEC2::DEC2(int num, int l) {
	*this = DEC2::DEC2(num);
	length = l;
}

DEC2::DEC2(string number)  {
	int a;
	length = 10000;
	digits.clear();
...
DEC2::DEC2(string number, int l) {
	*this = DEC2::DEC2(number);
	length = l;
}

DEC2::operator int() const {
	int sum = 0;
...
bool DEC2::operator==(const DEC2& y) {
	int f = 0;
	if (sign != y.sign)
		return 0;
...
bool DEC2::operator>(const int& y) {
	if (this->sign == 'p' && y < 0)
		return 1;
	if (this->sign == 'n' && y > 0)
		return 0;
	DEC2 big_int(2147483647), small_int(-2147483648);
	int x = 0;
	if (*this > big_int)
		return 1;
	if (*this < small_int)
...
DEC2 operator+(const DEC2& x, const DEC2& y) {
...
DEC2 operator-(const DEC2& x, const DEC2& y) {//uses addition operator
	DEC2 result, x2, y2;
	if (x.length > y.length)
		result.setLength(y.length);
	else
...
DEC2 operator*(const DEC2& x, const DEC2& y) {
	...
	if (x.sign != y.sign)//gets the sign of the result
		...
		result.sign='p';
	if (x2 == 0 || y2 == 0)
		return DEC2(0,result.length);
	int total = 0, carry = 0;
	DEC2 sum(0,result.length), temp(0,result.length),temp2(0,result.length);
	temp.digits.clear();
	temp2.digits.clear();
...
DEC2 operator/ (const DEC2& x, const DEC2& y) {
	...
	DEC2 result, z(0,1), x2, y2, x3(0,1), y3(0,1), x4(0,1), y4(0,1) , Zero(0,1), point(0,1);
	if (x.length > y.length)
		result.setLength(y.length);
	else
		result.setLength(x.length);
	...
	try{
		...
		int b = 0,cnt = 0;
		DEC2 xdiv;//remainder during division
		...
		if (x2 == y2){// if answer is a multiple of 1
			result.digits.push_back(1);
			////cout << point << endl;
			point = point + DEC2(x.digits.size() - y.digits.size());
			////cout << point << endl;
...
DEC2 operator+(const int& x, const DEC2& y){
	DEC2 x2 = x, result;
...
DEC2 operator+(const DEC2& x, const int& y){
	DEC2 y2 = y, result;
...
DEC2 operator^(const DEC2& x, const int& y){
	if (y == 0)
		return DEC2(1,x.length);
	DEC2 p = x;
	for (int i = y; i > 1; i--)
		p = p * x;
	return p;
}
void operator++(DEC2& y){
	DEC2 one(1,y.length);
	y = y + one;
}
void operator--(DEC2& y){
...
ostream& operator<< (ostream& os, const DEC2& num) {
	int a = 0;
	if (num.sign == 'n') os << '-';
...
istream& operator>> (istream& is, DEC2& num) {
	num.digits.clear();
	num.postdec.clear();
	int a = 0;
	int start = 0;
...
DEC2 DEC2::pwr(const int& y){
	if (y == 0)
		return DEC2(1,length);
	DEC2 p = *this;
	for (int i = y; i > 1; i--)
		p = p * *this;
	return p;
...
DEC2 DEC2::pwr(const int& a, const int& b){
	DEC2 result(0,length);
	result = (*this^a).root(b);
	return result;
}

DEC2 DEC2::sqrt(){
	DEC2 x(1,length), xcopy, xcopy2;
...
DEC2 DEC2::factorial(){
	...
	return DEC2(0,length);
}

DEC2 DEC2::sin(){
	DEC2 x = *this, result = *this, fact(3,length);
...
//DEC2 DEC2::cos(){
//	DEC2 x = *this, result, factn = 0, neg(-1);
...
DEC2 DEC2::cos(){
	DEC2 x = *this, result = (1,length), fact = (2,length);
