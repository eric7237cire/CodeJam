...
WeightedEdge<int> EdgeList[10000];  // format: weight, two vertices of the edge
int V, E;
vector<int> mstEdges;

//-1 means do the best MST, ignoring no edges

//Returns cost, or -1 if impossible
int doMST(int edgeIdIgnore)
{
	int nComp = V;
	FORE(v, 0, V)
	{
	...
	for (int i = 0; i < E; ++i) 
	{
		...
		int setU = findSet(front.u);
		int setV = findSet(front.v);
...
int main() 
{
	...
	FORE(t, 1, T)
	{
		...
		{
			int cost = doMST( mstEdges[i] );
			if (cost != -1 && cost < best)
				best = cost;
		}
				
		assert (best != numeric_limits<int>::max()); 
