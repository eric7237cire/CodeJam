...
int main() {
	...
	int T;
	while(4 == scanf("%d%d%d%d", &l, &w, &h, &angDeg))
	{
		if (l == 0 || w == 0 || h == 0)
		{
			printf("%.3lf mL\n", 0.0);
		...
		double maxAngle = atan( (double)h / l );
		
		const double ang1 = angDeg * pi / 180;
		//printf("%lf %lf\n", maxAngle, ang1);
		
		if (ang1 <= maxAngle)
		{
		
			double ang2 = (90 - angDeg) * pi / 180;
			
			//use law of sines 
			
			//base / sin90 = side1 / sin(ang1)
			//base / sin90 = l / sin(ang2)
			double base = l / sin(ang2);
			double side1 = base * sin(ang1);
			
			double areaTri;
			bool ok=areaTriangle(base, side1, l, areaTri);
			
			printf("%.3lf mL\n", (double)l*w*h - areaTri * w);
		} else {
		
			//triangle under box
			double ang2 = pi / 2 - ang1;
			//h / sin(ang1) = top / sin(90)
			// = baseTipped / sin(ang2)
			
			double top = (double)h / sin(ang1);
			double baseTipped =  top * sin(ang2);
			
			double areaTri;
			bool ok=areaTriangle(baseTipped, top, h,areaTri);
