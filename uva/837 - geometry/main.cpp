...
int main() {
	...
	while(T--)
	{
		...
		FOR(i, 0, nSeg)
		{
			...
			scanf("%lf%lf%lf%lf%lf", &x1, &y1, &x2, &y2, &coef);
			
			start.x = min(x1, x2);
			stop.x = max(x1, x2);
			
			start.coef = stop.coef = coef;
		...
		for(int i = 0; i < events.size() - 1; ++i)
		{
			...
			printf("%.3lf %.3lf %.3lf\n", events[i].x, events[i+1].x, light);
		}
		
		printf("%.3lf +inf 1.000\n", events[events.size() - 1].x);
