...
*/
	...
	bool onLine( const Point<T>& pt, double eps = tolerance)
	{
				...
				const Point<T>& b1,const Point<T>& b2)
{
	...
	if (aIsSeg && !bIsSeg)
	{
		return isColinear(a1, a2, b1) && isBetween(a1.x, a2.x, b1.x) && isBetween(a1.y, a2.y, b1.y);
	} else 	
	if (bIsSeg && !aIsSeg)
	{
		return isColinear(b1, b2, a1) && isBetween(b1.x, b2.x, a1.x) && isBetween(b1.y, b2.y, a1.y);
	}
...
void findCircle( PointD p1, PointD p2, PointD p3, double& radius, PointD& center)
{
	...
	assert(lineAB.onLine(midAB, 1e-7));
	assert(lineBC.onLine(midBC, 1e-7));
	...
	assert( lineMidABCenter.onLine(midAB, 1e-7) );
	assert( lineMidBCCenter.onLine(midBC, 1e-7) );
	assert( lineMidABCenter.onLine(center, 1e-7) );
	assert( lineMidBCCenter.onLine(center, 1e-7) );
...
template<typename T>
bool inOrOnTriangle( const Point<T>& p, const Point<T>& tri1,
...
//Heron's formula
bool areaTriangle(double a, double b, double c, double& area)
{
	double p = a + b + c;
	double s = p /2 ;
	double det = s * (s-a) * (s-b) * (s-c);
	if (det <= 0)
	{
		area = -1;
		return false;
...
//Law of cosines, returns angle opposite c
...
/*
Regular polgyon in a circle
Area of Polygon = n * Apothem^2 * tan(pi/n)
Area of Polygon = .5 * n * Radius^2 * sin(2 * pi/n)
		*/
...
char shapeType[100][10];
Point<int> points[100];
int data[100];
...
int main() {
	...
	FORE(t, 1, T)
	{
		...
		//scanf("%d%d", &nObjects, &nQueryPoints);
		next_int(nObjects);
		next_int(nQueryPoints);
		
		FOR(i, 0, nObjects)
		{
			next_string(shapeType[i]);
			next_int(points[i].x);
			next_int(points[i].y);
			next_int(data[i]);
			next_int(rgb[i][0]);
			next_int(rgb[i][1]);
			next_int(rgb[i][2]);
			//scanf("%s %d%d%d%d%d%d", shapeType[i], &points[i].x, &points[i].y,
				//&data[i], &rgb[i][0], &rgb[i][1], &rgb[i][2]);
			
		}
		
		FOR(q, 0, nQueryPoints)
		{
			...
			FOR(i, 0, nObjects)
			{
				if ( shapeType[i][0] == 'C' )
				{
					int ic = inCircle( pt, points[i], data[i] );
					...
					if (pt.y < points[i].y || pt.y > points[i].y + data[i])
						continue;
						
					if (pt.x == points[i].x || pt.x == points[i].x + data[i]
					|| pt.y == points[i].y || pt.y == points[i].y + data[i])
					{
						avg[0] = avg[1] = avg[2] = 0;
						n = 1;
						break;
			...
			{
				//printf("(255, 255, 255)\n");
				write_str("(255, 255, 255)\n");
			} else {
				//printf("(%d, %d, %d)\n", roundToInt( (double)avg[0] / n),
				write_char('(');
				write_int(round1(avg[0], n));
				write_str(", ");
				write_int(round1(avg[1], n));
				write_str(", ");
				write_int(round1(avg[2], n));
				write_str(")\n");
				//roundToInt((double)avg[1] / n), 
				//roundToInt((double)avg[2] / n) );
			}
		}
