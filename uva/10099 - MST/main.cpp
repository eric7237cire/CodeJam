...
int main() 
{
	...
	while(2 == scanf("%d%d", &V, &E) && (V|E) )
	{
		...
		EdgeList.assign(E, WeightedEdge<int>(0,0,0) );
		FOR(e, 0, E) 
		{
			scanf("%d%d%d", &EdgeList[e].u, &EdgeList[e].v, &EdgeList[e].weight);
		}
		
		int start, finish, nPeople;
		scanf("%d%d%d", &start, &finish, &nPeople);
 		...
 		for (int i = 0; i < E; ++i) 
		{
			...
			if (setU != setV)
			{
				...
				if (uf.findSet(start) == uf.findSet(finish))
				{
					int numTrips = nPeople  / (front.weight - 1);
					if (numTrips * (front.weight-1) < nPeople )
						++numTrips;
