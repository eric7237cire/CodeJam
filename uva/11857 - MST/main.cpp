...
public:
	...
	WeightedEdge(int _u, int _v, int _w) : u(_u), v(_v), weight(_w)
	{
	
	}
};

int operator<( const WeightedEdge& lhs, const WeightedEdge& rhs )
{
	NE_LT(weight);
	...
	while(2 == scanf("%d%d", &V, &E) && (V||E))
	{
		...
		{                           // for each edge, O(E)
			...
			{    // if no cycle
				...
				maxWeight = max(maxWeight, front.weight);
		...
		//printf("mst %llu total %llu\n", mst_cost, totalWeight - mst_cost); 
