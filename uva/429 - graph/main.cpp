...
int main() {
	...
	FORE(t, 1, T)
	{
		...
		vvi connections(idMap.size(), vi());
		
		for(int n = 0; n < idMap.size(); ++n)
		{
			for(int n2 = n+1; n2 < idMap.size(); ++n2)
			{
				string name1 = nameMap[n];
				string name2 = nameMap[n2];
				
				if (name1.length() != name2.length())
					continue;
					
				int diff = 0;
				for(int c = 0; c < name1.length(); ++c)
				{
					if (name1[c] != name2[c])
						++diff;
				}
				...
				if (diff == 1)
				{
					//cout << "Connecting " << name1 << n << " " << name2 << n2 << endl;
					connections[n].pb(n2);
					connections[n2].pb(n);
				}
			}
		...
		while( getline(cin, line) && line != "")
		{
			...
			string word2 = line.substr(pos+1, string::npos);
			
			int id1 = idMap[word1];
			int id2 = idMap[word2];
			//cout << "search " << word1 << id1 << " " << word2 << id2 << endl;
			
			//cost, id
			typedef pair<int, int> State;
			
			priority_queue<State, vector<State>, greater<State> > toVisit;
			set<int> visited;
			
			toVisit.push( State(0, id1) );
			bool finished = false;
			...
			while( !toVisit.empty() )
			{
				...
				if (contains(visited, cur.second))
					continue;
				...
				if (cur.second == id2)
				{
					cout << word1 << " " << word2 << " " << cur.first << endl;
					finished = true;
					break;
				}
				
				vi& adjList = connections[cur.second];
			
				FOR(i, 0, adjList.size())
				{
					toVisit.push( mp(cur.first + 1, adjList[i]) );
				}
			}
