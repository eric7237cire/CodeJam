...
//rectCount[rowIdx][colIdx][width] = # of rectangles with bottom right corner at colIdx and width
int rectCount[101][101][101];
#include <cstring>

int main() 
{

	while(2 == scanf("%d%d", &nRows, &nCols) && nRows)
	{
		...
		FORE(r, 1, nRows) 
		{
			...
			FORE(c, 1, nCols)
			{
				...
				if (ch == '0')
				{
					...
					FORE(w, 1, curWidth)
					{						
						//+ 1 is for flat rectangle of this row
						rectCount[r][c][w] = rectCount[r-1][c][w] + 1;
						sum += rectCount[r][c][w];
						
						//printf("r=%d c=%d w=%d  sum  = %d\n", r,c,w,rectCount[r][c][w]);
					}
				}
			}
		}
