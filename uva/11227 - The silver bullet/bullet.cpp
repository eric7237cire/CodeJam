...
int main() {
	...
	FOR(t, 0, T)
	{
		...
		{
			scanf("%lf %lf", &x, &y);
			ptsSet.insert(PointD(x, y));
		}
		
		vector<PointD> ptsList(ptsSet.begin(), ptsSet.end());

		int maxPtCount = min((int)ptsList.size(), 2);

		for(int i = 0; i < ptsList.size(); ++i)
		{
			PolarCmp<double> pc(ptsList[i] );

			vector<PointD> pts( ptsList.begin() + i + 1, ptsList.end() );
		    ...
		    while(pt2Idx < pts.size())
            {
                ...
                while( pt2Idx < pts.size() && isColinear( ptsList[i], pts[ptIdx], pts[pt2Idx] ) )
                {
                    ++pt2Idx;
                    ++count;
                }
		...
		printf("Data set #%d contains", t+1);

		if (ptsList.size() == 1) {
			printf(" a single gnu.\n");
		} else {
			printf(" %d gnus, out of which a maximum of %d are aligned.\n", ptsList.size(), maxPtCount);
		}
