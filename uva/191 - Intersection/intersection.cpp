...
int main() {
	...
	FOR(t, 0, T)
	{
		...
		scanf("%d%d", &lineA.x, &lineA.y);
		scanf("%d%d", &lineB.x, &lineB.y);

		PointI recA;
		PointI recB;

		scanf("%d%d", &recA.x, &recA.y);
		scanf("%d%d", &recB.x, &recB.y);

		bool ans = false;
		PointD inter;
		if ( isBetween(lineA.x, recA.x, recB.x) &&
			isBetween(lineA.y, recA.y, recB.y)) {
				ans = true;
		} else if ( isBetween(lineB.x, recA.x, recB.x) &&
			isBetween(lineB.y, recA.y, recB.y)) {
				ans = true;
		} else if (intersects( lineA, lineB, recA, PointI(recA.x, recB.y))) {
			ans = true;
		} else if (intersects( lineA, lineB, recA, PointI(recB.x, recA.y))) {
			ans = true;
		} else if (intersects( lineA, lineB, recB, PointI(recA.x, recB.y))) {
			ans = true;
		} else if (intersects( lineA, lineB, recB, PointI(recB.x, recA.y))) {
			ans = true;
		}
		cout << (ans ? "T" : "F") << endl;
	}
