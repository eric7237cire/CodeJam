...
int main() 
{
	...
	while( getline(cin, line) )
	{
		if (line == "#")
		{
			...
			prevLine = line = "";
			reset();
			letterExists.assign(26, false);
			continue;
		}
			
		for(int i = 0; i < line.length() && i < prevLine.length(); ++i)
		{
			int prev = prevLine[i] - 'A';
			int cur = line[i] - 'A';
			if ( prev != cur)
			{
				adjList[prev].pb(cur);
				letterExists[prev] = 1;
				letterExists[cur] = 1;
				//Anything later tells us nothing
				break;
			}
		}
