...
int bestDist[1000][2];
ii bestPts[1000][2];
int bestPtsIdx[1000][2];
ii coords[1000];
bool visited[1000];

void foundBestPoint(int v, int dist, int ptIdx)
{
	ii pt = coords[ptIdx];
	bestDist[v][1] = bestDist[v][0];
	bestPts[v][1] = bestPts[v][0];
	bestPtsIdx[v][1] = bestPtsIdx[v][0];
	
	bestDist[v][0] = dist;
	bestPts[v][0] = pt;
	bestPtsIdx[v][0] = ptIdx;
}

void foundSecondBestPoint(int v, int dist, int ptIdx)
{
	ii pt = coords[ptIdx];
	
	bestDist[v][1] = dist;
	bestPts[v][1] = pt;
	bestPtsIdx[v][1] = ptIdx;
}

bool compare( int v, bool is2nd, int dist, ii pt)
{
	if (bestDist[v][is2nd] > dist)
		return true;
	...
	//westernmost
	if (pt.first < bestPts[v][is2nd].first)
		return true;
	...
	//southernmost
	if (pt.second < bestPts[v][is2nd].second)
		return true;
...
int main() 
{
	while(scanf("%d", &V) == 1 && V)
	{
		...
		FOR(v, 0, V)
		{
			...
			bestDist[v][0] = bestDist[v][1] = numeric_limits<int>::max();
		}
		...
		FOR(v, 0, V)
		{
			FOR(v2, v+1, V)
			{
				int dist = distanceSq( coords[v], coords[v2]);
				if (compare(v, false, dist, coords[v2])) 
				{
					foundBestPoint(v, dist, v2);
				} else if (compare(v, true, dist, coords[v2])) 
				{
					foundSecondBestPoint(v, dist, v2);
				}
				
				if (compare(v2, false, dist, coords[v])) 
				{
					foundBestPoint(v2, dist, v);
				} else if (compare(v2, true, dist, coords[v])) 
				{
					foundSecondBestPoint(v2, dist, v);
				}
			}
		}
		...
		while(!toVisit.empty())
		{
			...
			toVisit.push(bestPtsIdx[cur][0]);
			toVisit.push(bestPtsIdx[cur][1]);
		}
