...
*/
	...
	while( getline(cin, s) )
	{
		int c = 0;
		 //string::npos
		int n1Start = s.find_first_of( "0123456789", 0 );
		int n1Stop = s.find_first_not_of( "0123456789", n1Start ) ;
		int opPos = s.find_first_of( "+*", n1Stop+1 );
		int n2Start = s.find_first_of( "0123456789", opPos+1 );
		int n2Stop = s.find_first_not_of( "0123456789", n2Start );
				
		string s1 = s.substr(n1Start, n1Stop - n1Start );
		string s2 = s.substr(n2Start, n2Stop - n2Start + 1);
		//cout << "[" << s1 << "]" << endl;
		
		BigInt x(s1);
		BigInt y(s.substr(n2Start, n2Stop - n2Start + 1));
