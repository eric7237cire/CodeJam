...
int main (void) {
	...
	while (cin >> a >> b) {
		...
		string_to_bignum(a, &start);
		string_to_bignum(b, &end);

		for (int i = 2; i < 5001; i++) {
			if ((compare_bignum(&start, &f[i]) == 0 || compare_bignum(&start, &f[i]) == 1) && (compare_bignum(&end, &f[i]) == 0 || compare_bignum(&end, &f[i]) == -1))
				result++;
		}
