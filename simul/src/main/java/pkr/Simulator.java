...
import poker_simulator.criteria.CriteriaEvaluator;
...
public class Simulator
{
	...
	public static void main(String[] args)
	{
		...
		// playerHoleCards.add("AA, KK, QQ, JJ, TT, 99, 88, 77, 66, 55, 44, 33,
		// 22, AK, AQ, AJ");
		...
		// playerHoleCards.add("A2+, K2+, Q2+, J2+, T2+, 92+, 82+, 72+, 62+,
		// 52+, 42+, 32+, 22+");
		// playerHoleCards.add("A2+, K2+, Q2+, J2+, T2+, 92+, 82+, 72+, 62+,
		// 52+, 42+, 32+, 22+");
		...
		try
		{
			...
			config = (Simul) context.createUnmarshaller().unmarshal(Resources.getResource("input.xml"));
	...
	public static Tree simulate(int TOTAL_SIMULATIONS, String preChosenFlop, List<String> playerHoleCards)
	{
		...
		CriteriaEvaluator ce = new CriteriaEvaluator(); 
		...
		// logOutput.debug("# of simulations {} of {}", simulNum,
		// TOTAL_SIMULATIONS );
	 ...
	 * 
	 * @param usedCards
	 *            si les cartes ont été utilisées
	 * @param range
	 *            l'eventail de cartes possible
	...
	private static HoleCards chooseValidAvailableCard(boolean[] usedCards, HoleCardsRange range)
	{
		...
		for (int i = 0; i < range.getCardsList().size(); ++i)
		{
			...
			if (!usedCards[hc.getCards()[0].index] && !usedCards[hc.getCards()[1].index])
			{
	...
	/**
	 * Choiser le flop, turn, et river parmi les cartes disponibles et mettres
	 * les résultats dans flopTurnRiver
	 * 
	 * @param usedCards
	 * @return
	 */
	...
	private static CompleteEvaluation[] simulateOneRound(boolean heroOnly, String flop, HoleCardsRange[] listRanges)
	{
		...
		for (int i = 0; i < listRanges.length; ++i)
		{
			...
			if (hc == null)
			{
				log.debug("No hole cards found for range {}  " + "prev holeCards {}", range, holeCards);
				return null;
			}
			...
			// log.debug("Hole cards ({}) {} idx 1 {} idx 2 {}", i,
			// holeCards[i], card1Index, card2Index);
		}
		 ...
		 * log.debug(
		 * "\nh1 {} eval: {}  \n h2  {} eval: {}  \n Flop {} turn {} river {}",
		 * holeCards[0], evals[0].toString(), holeCards[1], evals[1].toString(),
		 * flop, Card.listByIndex[ flopTurnRiver[3] ], Card.listByIndex[
		 * flopTurnRiver[4] ]); }
		 */
		return evals;
	}
