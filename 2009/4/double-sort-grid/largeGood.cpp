...
int nstate;
vector<int> state[max_nstate];
int F[max_nstate],G[max_nstate];
int expand[max_nstate][size+1][size+1];

/*
Just one step further. Let us focus not only the exact boundary 
for a letter but any monotone path. For any monotone path P and any
 letter c, define

dp[P][c] := the number of ways one can fill all the squares above 
the path P, using only the letters no greater than c, so that the 
upper part is doubly sorted, and any pre-filled letter in the upper 
part is respected.
For any monotone path except the most dominated one, we have one or
 more maximal points, those are the points where the path goes east
  then followed by a step upwards. In the second picture above, we 
  highlight a monotone path with its maximal points colored. To
   compute dp[P][c], we can divide the situation into two cases.
    (1) The letter c does not appear at all. There are dp[P][c-1] 
    ways to do so. (2) Otherwise, c must appear in at least one of
     the maximal points of P. For each non-empty subset of the
      maximal points, we can assign the letter c to them, reducing
       our task to dp[P'][c], where P' is a path that only differs
        from P in that subset of maximal points. 
        We use inclusion-exclusion formula on all 
        the non-empty subsets to compute
...
void DFS(vector<int> A,int prev)
{
    if (SIZE(A)==size)
    {
        state[nstate++]=A;
...
//  freopen("C-small-attempt2.in","r",stdin);freopen("C-small-attempt2.out","w",stdout);
    ...
    int testcase;
    scanf("%d",&testcase);
    memset(expand,255,sizeof(expand));
    for (int caseId=1;caseId<=testcase;caseId++)
    {
        ...
        for (int key=0;key<26;key++)
        {
            ...
            for (int k=0;k<size;k++)
            {
                ...
                {
                    G[i]+=F[i];
                    if (G[i]>=MODE) G[i]-=MODE;
                    //current path
                    int current=state[i][k];
                    
                    //previous in path.
                    int prev=(k==0)?size:state[i][k-1];
         printf("Current %d\n\nCurrent path [idx=%d] -- ", current, i);
         for(int j = 0; j < size; ++j) {
            printf("%d ", state[i][j]); 
        }           printf("\n");
                    //current row to previous
                    ...
                    printf("P is %d k is %d\n", p, k);
                        ...
                        if (expand[i][k][p]<0)
                        {
                        printf("Expanding nstate %d col %d row %d  expand[i][k][p] = %d\n", i, k, p, expand[i][k][p]); 
                            vector<int> newstate=state[i];
                            //Move the row down by one
                            newstate[k]=p+1;
                            
                            //M converts path back to path id
                            expand[i][k][p]=M[newstate];
                        }
                        int newId=expand[i][k][p];
                        G[newId]+=F[i];
                        printf("Adding new path id %d.  F[i] %d  G[newId] %d\n", newId, F[i], G[i]);
                        if (G[newId]>=MODE) G[newId]-=MODE;
                    }
                }
                memcpy(F,G,sizeof(F));
                ...
                for (int i=0;i<nstate;i++) if (F[i]>0)
                {
                    printf("path # %d ",i);
                    printf("{");
                    for (int k=0;k<size;k++) printf("%d ",state[i][k]);
                    printf("}  %d\n",F[i]);
                }

            }
        }
        for (int i=0;i<size;i++) E[i]=size;
        printf(" %d\n",F[M[E]]);
        fflush(stdout);
    }
    return 0;
