    ...
    Console.WriteLine(total);
        ...
        for (long v = 1; v <= limit; ++v) {
            for (long u = v + 1; u <= limit; ++u) {
                 ...
                 * if and only if m and n are coprime and m − n is odd.
                 *  If both m and n are odd, then a, b, and c will be even,
                 *   and so the triple will not be primitive; however,
                 *    dividing a, b, and c by 2 will yield a primitive 
                 *    triple if m and n are coprime
                 */
                ...
                while (true) {
                    ...
                    //PythagTriple trip = new PythagTriple(k * a, k * b, k * c);

                    if (k * len > lenLimit)
                        break;
                   
                    counts[Ints.checkedCast(k*len)]++;
                    ++k;
                }
