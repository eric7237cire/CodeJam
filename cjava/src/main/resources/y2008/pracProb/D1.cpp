...
double f(int mask,int cur,int b) {
  ...
  if(ret==-1) {
	...
	double val;
	if(j==cur) {
	  ret = min(ret, f(tmp,j,b||bad[i])+M[cur][i]);
	}
	else if(bad[i] || b) {
	  ret = min(ret, f(tmp,j,0)+M[cur][i]+(abs(P[cur])+abs(P[j]))*price);
	}
	else {
	  ret = min(ret, f(tmp,j,0)+M[cur][i]+abs(P[j]-P[cur])*price);
	}
      }
...
int main(){
  ...
  for(int C=1; C<=T; C++) {
    ...
    */
    ...
    int ii;
    REP(i,m) {
      sol = min(sol, f((1<<n)-1,i,0)+abs(P[i])*price);
    }
    printf("Case #%d: %.7f\n",C,sol);
  }
}
