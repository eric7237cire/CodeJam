...
void solve(int H,int T)
{
	...
	for (int i=H;i<=T;i++) if (fabs(P[i].first-sp)<=R/2.0)
		if (i<=M)
			QL.push_back(MP(P[i].second,i));
		else
			QR.push_back(MP(P[i].second,i));
	sort(QL.begin(),QL.end());
	sort(QR.begin(),QR.end());
	for (int step=0;step<2;step++)
	{
		for (int i=0,k=0;i<SIZE(QL);i++)
		{
			for (;k<SIZE(QR) && QR[k].first<QL[i].first-R/2.0;k++);
			int d=k;
			for (;d<SIZE(QR) && QR[d].first<=QL[i].first+R/2.0;d++);
			for (int a=k;a<d;a++)
				for (int b=a+1;b<d;b++)
