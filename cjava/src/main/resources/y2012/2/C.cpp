...
bool solve(int s,int t)
{
	...
	if (s>=t) return true;
	vector<int> pos;
	for (int i=s;i<t;i++) if (target[i]==t) pos.push_back(i);
	if (SIZE(pos)==0) return false;
	for (int i=pos[0]+1;i<t;i++) if (target[i]>t) return false;
	int last=s;
	for (int i=0;i<SIZE(pos);i++)
	{
		int current=pos[i];
		if (!solve(last,current)) return false;
		for (int k=last;k<=current;k++) R[k]-=(current-k+1);
		last=current+1;
	}
	return true;
}
...
//	freopen("C-small-attempt2.in","r",stdin);freopen("C-small-attempt2.out","w",stdout);
	...
	for (int case_id=1;case_id<=testcase;case_id++)
	{
		...
		if (solve(0,n-1))
		{
			...
			printf("\n");
			/*
			for (int i=0;i<n-1;i++) for (int j=i+1;j<n;j++) if (j!=target[i])
			{
				int64 t=outer(i,R[i],j,R[j],target[i],R[target[i]]);
				if (t<=0)
				{
					printf("ERROR\n");
					exit(0);
