...
double solve(int n,int c)
{
	...
	for (int i=0;i<=c;i+=d)
	{
		double t1=solve(n-1,c-i);
		double t2=solve(n-1,c+i);
		double t=t1*(1-p)+t2*p;
		if (t>ret) ret=t,rkey=i;
	}
	key[n][c]=rkey;
	return ret;
}
...
//	freopen("..\\C-small-attempt0.in","r",stdin);freopen("..\\C-small-attempt0.ans","w",stdout);
//	freopen("..\\C-small-attempt1.in","r",stdin);freopen("..\\C-small-attempt1.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		target=two(n);
		double rate=1e6/target;
		double ret=solve(n,(int)(x0/rate+1e-11));
		printf("%.6lf\n",ret+1e-13);
		fflush(stdout);
	}
	return 0;
