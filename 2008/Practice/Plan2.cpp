      ...
      if(A[mask][store]<0.0)
      {
            ...
            if((mask&dupa)==mask)
            {
                  ...
                  REP(i,m) if((mask&T[i])==mask)
                  {
                        ...
                        A[mask][store] = min(A[mask][store], cost);
                  }
            }
            else
            {
...
*/
                  ...
                  {
                        REP(i,m) A[mask][store] = min(A[mask][store], dist(i,store) + G[store][j] + gogo(mask - (1<<j),i));
                  }
            }
      }
      return A[mask][store];
...
//oblicz pojedyncze cykle
            ...
            FOR(i,1,(1<<n)-1) {A[i][m] = 1e100;REP(j,m) A[i][m] = min(A[i][m], dist(m,j) + gogo(i,j));}
//scal cykle
            B[0] = 0.0;
            FOR(i,1,(1<<n)-1)
            {
                  ...
                  {
                        start = ((start  | (~i))+1) & i;
                        B[i] = min(B[i], A[start][m] + B[i&(~start)]);       
                  }            
            }       
            printf("Case #%d: %.7lf\n",tt,B[(1<<n)-1]);
      }
