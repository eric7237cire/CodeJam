...
public:  
    ...
    for(unsigned int ch = 0; ch < MAX_LETTER; ++ch) 
    {
      ...
      for(unsigned int i=0; i<row_pos_hits.size(); ++i) {
        for(unsigned int j=0; j<next_row_pos_hits.size(); ++j) {
          const unsigned int pos1 = row_pos_hits[i];
          const unsigned int pos2 = next_row_pos_hits[j];
          if (pos1==pos2) {
            continue;
          }
  ...
  inline unsigned int getEdgeWeight(unsigned int fromNode, unsigned int toNode) const
  {
...
{
public:
  typedef int(* M_ARRAY) [MAX_K][1 << MAX_K];
  typedef int(& M_ARRAY_REF) [MAX_K][1 << MAX_K];
  static M_ARRAY m_p;
...
public:
  ...
  DP(const Graph& graph, unsigned int k) : graph(graph), stop(0), cache_hits(0), k(k) {
    ...
    if (!m_p) {
      m_p = (M_ARRAY) new int[MAX_K][1 << MAX_K];
    }
    
    reset_cache();
    
    assert(m[0][0] == -1);
    assert(m[MAX_K-1][0] == -1);
    ...
    assert(powers_2[15] == 1 << 15);
    assert(powers_2[k] == 1U << k);
  }
  
  unsigned int distance(const unsigned int startNode, unsigned int visited) 
  {
    int boundaryBonus = graph.getBoundaryBonus(startNode, endNode);
    
    visited |= powers_2[startNode] | powers_2[endNode];
    ...
    if (visited == powers_2[k] - 1) {
      ...
      debug("END %d + %d\n", ret, boundaryBonus);
      m[startNode][visited] = ret+boundaryBonus;
    ...
    for(unsigned int pos = 0; pos < k; ++pos)
    {
      if ( (visited & powers_2[pos]) == 0) {
        ...
        const unsigned int weight = boundaryBonus + graph.getEdgeWeight(startNode, pos) + distance_nested(pos, visited | powers_2[pos] );
    ...
    m[startNode][visited] = max_weight;
    return max_weight;
  }
  
  unsigned int distance_nested(const unsigned int startNode, unsigned int visited) 
  {
    int cached_value = m[startNode][visited];
    ...
    //debug("Cache miss Start Node: %d, End Node: %d, Visited: %d\n", startNode, endNode, visited);
    ...
    for(unsigned int pos = 0; pos < k; ++pos)
    {
      if ( (visited & powers_2[pos]) == 0) {
        ...
        const unsigned int weight = graph.getEdgeWeight(startNode, pos) + distance_nested(pos, visited | powers_2[pos] );
    ...
    //debug("RETURN Start Node: %d, End Node: %d, Visited: %d, returning %d\n", startNode, endNode, visited, max_weight);
    m[startNode][visited] = max_weight;
    return max_weight;
  }
...
DP::M_ARRAY DP::m_p = (M_ARRAY) new int[MAX_K][1 << MAX_K];
...
void test() 
{
  ...
  DP dp2(g2, 5);
  dp2.endNode = 4;
  assert(dp2.distance(0, 0) == 2);
  assert(dp2.distance(1, 0) == 1);
  }
}
...
void do_test_case(int test_case, ifstream& input)
{
  ...
  for(unsigned int end_point=0; end_point<k; ++end_point) {
    dp.reset_cache();
    dp.endNode = end_point;
    for(unsigned int j=0; j<k; ++j) {
      if(end_point==j) {
        continue;
      }
      
      max_reduction = max<unsigned int>( dp.distance(j, 0), max_reduction );      
    }
  }
